...
def freqs (l : List β) :=
  let update (tbl : Batteries.HashMap β Int) (i : β) :=
    if tbl.contains i then tbl.insert i (tbl.find! i + 1)
    else tbl.insert i 1
  List.foldl (fun tbl i => update tbl i) Batteries.HashMap.empty l

def unionFreqs (l : List (List β)) :=
  let update (tbl : Batteries.HashMap β Int) (i : β) :=
    if tbl.contains i then tbl.insert i (tbl.find! i + 1)
    else tbl.insert i 1
  let updateMany (tbl : Batteries.HashMap β Int) (l : List β) :=
    l.foldl update tbl
  (l.foldl updateMany Batteries.HashMap.empty)
